...
forholdet mellom registrerte individer og det man basert på uavhengige kilder
(f.eks.
...
individer i det aktuelle året. \oldtext{Validitet angir graden av korrekthet for
én eller et sett av opplysninger gjerne basert på en sammenligning mot andre,
uavhengige kilder.} \newtext{Man bør kunne si noe om alle dimensjoner ved
datakvalitet:
\begin{itemize}
  ...
  \item relevans (fyller registerets variable et kunnskapsbehov, nærhet til
    feltet)
  \item nytteverdi (entydige, lettfattelige og godt beskrevne variabler samt
...
\guide{
	...
	dekningsgradsanalysen. Dekningsgrad på sykehus- eller
        enhetsnivå bør oppgis og gjerne på gruppenivå (for eksempel
        diagnosegrupper) der dette er relevant. Resultater av
        frafallsundersøkelser bør om mulig også angis. Det betyr å
        undersøke om populasjonen som ikke er inkludert i registeret er
        sammenlignbar med populasjonen i registeret. Hvis populasjonene er like
        indikerer det at man i registeret har et tilfeldig utvalg og at
        manglende registreriner derfor er mindre kritisk.
	}}
...
\guide{\newtext{Validitet angir graden av korrekthet for én eller et sett av
opplysninger gjerne basert på sammenligning mot andre, uavhengige kilder.}
Beskriv hvilke eksterne kilder som er benyttet og hvordan
dette er sammenholdt med registerets data. Beskriv metoden for
valideringsstudien\newtext{, eksempelvis:
\begin{itemize}
  \item hvilke sykehus, avdelinger eller enheter som ble validert,
  \item hvilke variabler som ble validert,
  \item tidsrom som dekkes av de data som benyttes i studien,
  \item størrelsen på andelen av hendelser eller pasienter som ble benyttet i
    studien, og
...
\guide{\newtext{Følgende vil være relevant å ta med i vurdering av
datakvalitet:}
\begin{itemize}
  \item Angi viktige funn og en vurdering av resultatene fra
    \oldtext{valideringsanalyser} \newtext{alle dimensjoner av datakvalitet der
    dette er relevant og utført; validitet, relibialitet og kompletthet. Et
    eksempel er} \oldtext{slik som} funn av systematiske feil \newtext{eller}
    \oldtext{,} tilfeldige feil og antatt effekt dette har på
    \oldtext{validitet} \newtext{datakvalitet} for hele registeret.
  \item Angi graden av kompletthet for sentrale variabler.
  \item \newtext{Vis registreringsforsinkelse (aktualitet), det vil si tid fra
    hendelse (eksempelvis innleggelse eller utskriving) til hendelsen er
    registrert i registeret}
  \item \newtext{Gi en vurdering av nytteverdi og relevans, herunder vise til
    komplett variabelbeskrivelse og begrunnelse for alle variabler i
    registeret.}
  \item \newtext{Gi en vurdering av sammenlignbarhet over tid for
    enkeltvariabler i registeret og oppgi områder i registeret som eventuelt er
    kjent ikke-sammelignbare og/eller vis til dokumentasjon av endinger i
    variabelsettet}
\end{itemize}}
...
for ytterligere beskrivelse. \newtext{Beskrivelse av
kvalitetsindikatorer bør tydelig angi hvordan disse er definert, eksempelvis
hvilke pasientgrupper som inngår, om man ser på alle hendelser per pasient eller
bare de nyeste hendelser, om man bare ser på pasienter som er i live ved en gitt
dato etc.} \oldtext{Indikér} \newtext{Angi gjerne} hvilke av disse
