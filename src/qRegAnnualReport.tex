...
inn. I kapittel \ref{cha:kva} og \ref{cha:dat} er begrepet ”institusjon” brukt.
...
relevante for klinisk kvalitetsforbedring og resultater viktige for vurdering av
...
\guide{Status og evaluering av registerets  datakvalitet. Dekningsgraden angir
forholdet mellom registrerte hendelser/individer og det man basert på
uavhengige kilder (f.eks.
...
hendelser/individer i det aktuelle året. Man bør kunne si noe om alle
dimensjoner ved datakvalitet:
\begin{itemize}
  \item kompletthet (dekningsgrad og ufullstendige registreringer)
  \item validitet (gyldighet)
  \item sammenlignbarhet (registreres egentlig det samme på alle avdelinger)
  \item aktualitet (registreringsforsinkelse)
  \item reliabilitet (variasjon ved gjentatte målinger)
  \end{itemize}
Se
\href{https://www.kvalitetsregistre.no/om-datakvalitet\#kvalitetsdimensjoner}
{Valideringshåndboken}\footnote{\url{https://www.kvalitetsregistre.no/om-datakvalitet\#kvalitetsdimensjoner}}
...
per institusjon og år, samt totalt.}
...
\guide{Oppgi om registeret samler data fra HF i alle regioner og andel av
aktuelle sykehus/avdelinger som sender data til registeret.}

\section{Dekningsgrad}\label{sec:obs}
\guide{Angi resultat av dekningsgradsanalyse på individnivå for hele registeret
totalt og tidspunkt for gjennomføring av dekningsgradsanalysen. Dekningsgrad på
sykehus- eller enhetsnivå bør oppgis og gjerne på gruppenivå (for eksempel
diagnosegrupper) der dette er relevant.

Resultater av frafallsundersøkelser bør om mulig også angis. Det betyr å
undersøke om populasjonen som ikke er inkludert i registeret er sammenlignbar
med populasjonen i registeret. Hvis populasjonene er like indikerer det at man
i registeret har et tilfeldig utvalg og at manglende registreriner derfor er
mindre kritisk.}

\section{Prosedyrer for intern sikring av datakvalitet}\label{sec:sik}
\guide{Beskriv infrastruktur og rutiner (eksempelvis opplæring,
informasjonsarbeid, tekniske støttesystemer) som ivaretar riktige og komplette
data fra innregistrerende enheter og i registeret sentralt.}

\section{Metode for validering av data i registeret}\label{sec:metval}
\guide{Validitet angir graden av korrekthet for én eller et sett av opplysninger
gjerne basert på sammenligning mot andre, uavhengige kilder. Beskriv hvilke
eksterne kilder som er benyttet og hvordan dette er sammenholdt med registerets
data. Beskriv metoden for valideringsstudien, eksempelvis:
\begin{itemize}
  \item hvilke sykehus, avdelinger eller enheter som ble validert samt hvordan
    og hvorfor disse ble valgt,
  \item hvilke variabler som ble validert,
  \item tidsrom som dekkes av de data som benyttes i studien,
  \item hvor stor andel av hendelser eller pasienter som ble benyttet i studien,
    og
  \item andre hensyn som er tatt under valideringsstudien.
\end{itemize}} 

\section{Vurdering av datakvalitet}\label{sec:valdat}
\guide{Følgende vil være relevant å ta med i vurdering av datakvalitet:
\begin{itemize}
  \item Angi viktige funn og en vurdering av resultatene fra alle dimensjoner
    av datakvalitet der dette er relevant og utført; validitet, relibialitet og
    kompletthet. Et eksempel er funn av systematiske feil eller tilfeldige feil
    og antatt effekt dette har på datakvalitet for hele registeret.
  \item Angi graden av kompletthet for sentrale variabler.
  \item Vis registreringsforsinkelse (aktualitet), det vil si tid fra hendelse
    (eksempelvis innleggelse eller utskriving) til hendelsen er registrert i
    registeret (registreres egentlig det samme på alle avdelinger)
  \item Gi en vurdering av sammenlignbarhet (registreres egentlig det  samme på
    alle avdelinger og over tid)  for enkeltvariabler i registeret og oppgi
    områder i registeret som eventuelt er kjent ikke-sammelignbare og/eller vis
    til dokumentasjon av endinger i variabelsettet
    \item Si noe om reliabiliteten, dvs. treffsikkerheten eller variasjonen ved
      gjentatte målinger
\end{itemize}}
...
\guide{Beskriv de viktigste variabler/indikatorer som angir grad av kvalitet
(struktur-, prosess- og resultatmål). Se også
...
for ytterligere beskrivelse. Av kvalitetsindikatorbeskrivelsen skal det tydelig
framgå hvordan indikatorene er definert. For eksempel om det gjøres utvalg på
subgrupper av registerpopulasjonen og om man ser på alle hendelser eller bare
nyeste hendelse for hver pasient. Angi gjerne hvilke av disse indikatorene som
vil egne seg som nasjonale kvalitetsindikatorer.}
...
\emph{best practice} beskrives. Resultater i seg selv beskrives i kap.
\ref{cha:res}.}

\section{Identifisering av kliniske forbedringsområder}\label{sec:ide}
\guide{Beskriv kliniske forbedringsområder som er identifisert på bakgrunn av
analyser fra registeret i løpet av de siste tre årene.}

\section{Tiltak for klinisk kvalitetsforbedring initiert av
registeret}\label{sec:brures}
\guide{Angi eventuelle kliniske kvalitetsforbedringsarbeider basert på data fra
registeret (initiert av registeret eller kliniske enheter som registrerer i
registeret) for de siste tre årene. Beskriv hvordan registeret bidrar til
oppstart og gjennomføring av klinisk kvalitetsforbedringsarbeid lokalt hos
innregistrerende institusjoner.}
...
\guide{Status og evaluering av formidlingsform og -frekvens av resultater fra
registeret. Resultater i seg selv beskrives i kap. \ref{cha:res}.}
...
\guide{Beskriv form og frekvens, og hvordan denne informasjonen formidles til
...
\guide{Beskriv hvor/til hvem, form og frekvens. Resultater i seg selv beskrives
i kap. \ref{cha:res}.}
...
vitenskapelige konferanser, utdanning av doktorgradsstipendiater eller lignende
i løpet av de siste to år.}
...
for neste kalenderår. Punktlisten under kan brukes som et utgangspunkt og hjelp
til hva som kan inngå i en slik beskrivelse. For ytterligere forklaring av de
ulike punktene, se respektive kapitler under Del \ref{par:rap}:
\nameref{par:rap}.}
