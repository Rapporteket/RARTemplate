...
\st{Dekningsgrad oppgis på institusjons- og individnivå. Dersom et register ett år mottar registreringer fra tre av totalt fire mulige institusjoner blir dekningsgraden på institusjonsnivå 75\%}.
Dekningsgraden \st{på individnivå} angir
...
\guide{Oppgi andel av aktuelle sykehus/avdelinger som sender data til
registeret.}
...
\guide{Beskriv hvilke eksterne kilder som \hl{skal} \st{er} \hl{benyttes} \st{benyttet} og hvordan
dette \hl{skal} \st{er} sammenholdes \st{sammenholdt} med registerets data (design/subsampling).} 
...
\begin{longtable}{rp{10cm}lcc}
  ...
  \endlastfoot
   & \textbf{Stadium 2} & & \\
  1 & Er i drift og samler data fra \st{HF i alle helseregioner} \hl{alle
      aktuelle HF}
  ...
  8 & \hl{Har dekningsgrad over 60 \%}
    & \ref{sec:met}, \ref{sec:endek}, \ref{sec:obs} & \Square& \Square \\
  9 & Registrerende enheter \hl{skal} utover årsrapport kunne \st{kan} få
      utlevert egne aggregerte og nasjonale resultater
    & \ref{sec:resfag}, \hl{{\ref{sec:resled}}}, \st{{\ref{sec:off}}} & \Square & \Square \\
  ...
  19 & Registrerende enheter har løpende (on-line) tilgang til oppdaterte egne
      og nasjonale resultater & \ref{sec:resfag}, \ref{sec:resled} & \Square
    & \Square \\
